...
const createHtmlResponse = (body, { debug = false } = {}) => '<!DOCTYPE html>\n'
  ...
  + `<pre class='flex flex-col flex-1 h-screen'>${body}</pre>\n`
  + '</body>\n'
  + '</html>\n';

const uploadFileResponse = (path) => '<section class="bg-slate-50 py-12 px-4">'
  + `<h1 class="sm:text-3xl text-lg font-sans mb-4 text-center sm:text-left whitespace-normal"><strong>Upload files</strong></h1>`
...
const generateDeleteButton = (file, deleteFile, label = 'Remove') => (!deleteFile === true
  ? ''
  : `<a target="_self" class="ml-1 text-blue-600 text-sm px-3 py-1 bg-blue-100 rounded-full inline-flex space-x-1" href="${`${encodeURIComponent(
    file,
  )}?delete=true&file=${encodeURIComponent(file)}`}">`
